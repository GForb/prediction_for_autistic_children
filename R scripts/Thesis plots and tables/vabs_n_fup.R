...
analysis_data_wide <- analysis_data_wide |> mutate(out_partially_complete = case_when(
  ...
  !out_all_complete & (!is.na(out_vabs_dls_ae) | 
                       !is.na(out_vabs_com_ae) | 
                       !is.na(out_vabs_soc_ae)) ~ TRUE,
  TRUE ~ FALSE))
...
table_data <- by_study_accounting  |> mutate(no_fu = loss_to_follow_up + partially_completed_outcome) |> 
  ...
  pivot_longer(cols = c(included, not_included, outcome_out_of_range, no_fu), names_to = "status", values_to = "n") |> 
...
write_csv(table_data, file.path(thesis_tables, "vabs_fu_status.csv"))
