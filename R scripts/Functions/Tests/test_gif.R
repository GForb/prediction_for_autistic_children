...
gif_plots <- make_raincloudplot_wave(lsac_data, "sdq_tot_p", "Total SDQ", "springgreen")
gif <- create_gif(gif_plots)
image_write(image = gif, path = file.path(outputs, "lsac_gif.gif"))



gif_plots2 <- make_raincloudplot_wave(lsac_data, "sdq_tot_p", "Total SDQ", "springgreen", var_metadata = var_metadata)
