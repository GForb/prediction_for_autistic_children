...
Tbale shows estimate (95% CI) [tau] where tau is the square root of the between study variance in estimates. 
...
full_results <- readRDS(here::here(results_folder, "results_meta_analysis.rds")) |> 
  select(-starts_with("tau"), -starts_with("meta_analysis"), -analysis_name) |> 
  filter(predictor_set == "pred_full")
  ...
  for(myOut in outcomes){
    ...
    full_results |>   
      ...
      select(-outcome) |> 
      select( model ,intercept = intercept_est_method, r_squared, everything() , -predictor_set) |> 
      flextable::flextable() |>
      flextable::autofit() |> 
      flextable_to_rmd()
  }
...
model_name <- "mt_ri_study_rs"
    ...
    full_results |>   
      arrange(intercept_est_method) |>
      filter(intercept_est_method == intercept_est, model == model_name) |> 
      select(-intercept_est_method, -model, -predictor_set) |> 
      select( outcome,  r_squared, everything()) |> 
      flextable::flextable() |>
      flextable::autofit() |> 
      flextable_to_rmd()
...
report_model(analysis_name = model_full_name, results_folder = results_folder)
```
