cap prog drop get_all_preds_ri_study
prog define get_all_preds_ri_study
syntax varname, model_code(passthru) pred_var_name(passthru) intercept_est(passthru)  [ random_split  model_options(passthru) intercept_value(passthru)]
	...
	foreach fold in `folds' {
		...
		di "Fold:  `fold'"
		preserve
		get_hold_out_pred_ri `varlist', hold_out_fold(`fold') `model_code' `pred_var_name'  `model_options' `random_split' `intercept_est'
